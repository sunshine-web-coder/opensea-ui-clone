...
import nMLists from "../../allData/explore.json";
import resourcesLinks from "../../allData/resourcesLinks.json";
import { useState } from "react";
import DarkModeSwitch from "../darkModeSwitch/DarkModeSwitch";

const MobileHeader = () => {
  const [menuOneShow, setMenuOneShow] = useState(false);
  const [menuTwoShow, setMenuTwoShow] = useState(false);
  const [menuThreeShow, setMenuThreeShow] = useState(false);

  return (
    <nav className="mobileNavBar">
      <ul>
        <li onClick={() => setMenuOneShow(!menuOneShow)}>
          <div>
            <ExploreOutlinedIcon className="MobileIcons" />
            Explore
          </div>
          <KeyboardArrowRightOutlinedIcon className="MobileIcons" />
        </li>
        {menuOneShow && (
          <div className="nextMenu">
            <div className="nMnHeader" onClick={() => setMenuOneShow(false)}>
              <KeyboardArrowLeftOutlinedIcon className="MobileIcons" />
              Explore
            </div>
            <div className="nextMenuList">
              <ul>
                {nMLists.map((nMList) => {
                  return (
                    <li>
                      <Link to={nMList.explorePath}>
                        <img src={nMList.img} alt="" />
                        {nMList.title}
                      </Link>
                    </li>
                  );
        ...
        <li onClick={() => setMenuTwoShow(!menuTwoShow)}>
          <div>
            <EqualizerOutlinedIcon className="MobileIcons" />
            Stats
          </div>
          <KeyboardArrowRightOutlinedIcon className="MobileIcons" />
        </li>
        {menuTwoShow && (
          <div className="nextMenu">
            <div className="nMnHeader" onClick={() => setMenuTwoShow(false)}>
              <KeyboardArrowLeftOutlinedIcon className="MobileIcons" />
              Stats
            </div>
            <div className="nextMenuList">
        ...
        <li onClick={() => setMenuThreeShow(!menuThreeShow)}>
          <div>
            <LibraryBooksOutlinedIcon className="MobileIcons" />
            Resources
          </div>
          <KeyboardArrowRightOutlinedIcon className="MobileIcons" />
        </li>
        {menuThreeShow && (
          <div className="nextMenu nextMenu3">
            <div className="nMnHeader" onClick={() => setMenuThreeShow(false)}>
              <KeyboardArrowLeftOutlinedIcon className="MobileIcons" />
              Resources
            </div>
            <div className="nextMenuList">
              <ul>
                {resourcesLinks.map((rLinks) => {
                  return(
                    <li key={rLinks.id}><Link to={rLinks.resourcesPath}>{rLinks.title}</Link></li>
                  )
                })}
              </ul>
            </div>
        ...
        <li className="darkmodeSwith">
          <div>
            <BedtimeIcon className="moonIcon MmoonIcon" />
            Night mode
          </div>
          <DarkModeSwitch />
        </li>
        <div className="M_site_btn">
          <button className="site_btn M_site_button">Connect wallet</button>
        </div>
      </ul>
      <div className="MfooterNav">
        <div className="in_MfooterNav">
          <svg
